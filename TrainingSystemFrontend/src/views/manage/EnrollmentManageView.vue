...
const { data: roster, loading: rosterLoading, refresh: refreshRoster } = useAsyncData(() =>
  enrollmentService.getRoster(courseId),
)
const { data: departments } = useAsyncData(() => departmentService.getAll())
const { data: allUsers } = useAsyncData(() => userService.getAll())

const enrolledUserIds = computed(() => new Set((roster.value || []).map((e) => e.userId.id)))
  ...
  <div class="page">
    <div class="detail-header">
      ...
      <div class="detail-heading">
        ...
        <div class="detail-meta">{{ roster?.length ?? 0 }} người đã ghi danh</div>
      </div>
    </div>
      ...
      <div class="enroll-side">
        ...
        <div class="card">
          ...
          <div v-else class="employee-picklist">
            <label v-for="u in availableEmployees" :key="u.id" class="employee-pick-row">
