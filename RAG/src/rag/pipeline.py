...
class RAGPipeline:
    def __init__(self, settings: Settings | None = None):
        ...
        client = genai.Client(api_key=self.settings.api_key, http_options=http_options)
    ...
    def ingest(self) -> int:
        ...
        if not documents:
            raise RuntimeError(f"Không tìm thấy tài liệu nào trong {self.settings.raw_dir}.")
        chunks = chunk_documents(documents, self.settings.chunk_size, self.settings.chunk_overlap)
        vectors = self.embedder.embed_documents([c.text for c in chunks])
        metadatas = [{"text": c.text, "source": c.source, "index": c.index} for c in chunks]
